...
To expose a database to Common Fate, you will need to register them with our `proxy-resource-aws-rds` Terraform module. Some examples have been provided below for configuring a mysql and postgres database.
...
module "proxy-resource-aws-rds-sql" {
  source  = "common-fate/proxy-resource-aws-rds/commonfate"
  proxy_id=module.proxy.proxy_id
  app_url = "https://commonfate.example.com" // Your Common Fate App url
  rds_name="us-west-2-prod" //used to lookup the rds database with the AWS datasource
...
module "proxy-resource-aws-rds-postgres" {
  source  = "common-fate/proxy-resource-aws-rds/commonfate"
  proxy_id=module.proxy.proxy_id
  app_url = "https://commonfate.example.com" // Your Common Fate App url
  rds_instance_identifier="us-west-2-prod" //used to lookup the rds database with the AWS datasource
  rds_security_group_id="sg-12345678912" // database security group id
  users = [{
     name                      = "Root"
     username                  = aws_db_instance.postgres_db.username
     passwordSecretsManagerARN = aws_db_instance.postgres_db.master_user_secret[0].secret_arn
  }]
...
The `proxy-resource-aws-rds` module will create the necisary IAM policies to allow the proxy service to read the SSM secrets, create the security group rule allowing the proxy to access the database, and registering the database with Common Fate so it knows of its existence.

The AWS Secrets Manager secret value referenced by `passwordSecretsManagerARN` **must** be a key/value pair containing a `password` key. For example, the following secret in JSON format is valid:

```json
{ "username": "postgres", "password": "postgrespassword" }
```

Secrets created by AWS RDS follow this format.
...
You can now create an access workflow and availabilities, these resources make a single database and role available for requesting:
...
resource "commonfate_aws_rds_database_availability" "mysql" {
  ...
  aws_rds_database_id          = "arn:aws:rds:us-west-2:12345678912:db:example-mysql-db/test"
...
resource "commonfate_aws_rds_database_availability" "postgres" {
  workflow_id                  = commonfate_access_workflow.rds.id
  aws_rds_database_id          = "arn:aws:rds:us-west-2:12345678912:db:example-mysql-db/test"
  aws_rds_database_user_id     = "postgres@arn:aws:rds:us-west-2:12345678912:db:example-postgres-db/postgres"
  aws_identity_store_id        = local.identity_store_id
}
resource "commonfate_aws_rds_database_availability" "postgres_read_only" {
  workflow_id                  = commonfate_access_workflow.rds.id
  aws_rds_database_id          = "arn:aws:rds:us-west-2:12345678912:db:example-mysql-db/test"
