...
The `proxy-resource-aws-rds` module will create the necessary IAM policies to allow the proxy service to read the SSM secrets, create the security group rule allowing the proxy to access the database, and registering the database with Common Fate so it knows of its existence.
