...
The proxy service captures audit logs of all SQL commands executed by the user during their session which can bew viewed in real-time.
...
The user then uses the Granted CLI `granted rds proxy` to begin portforwarding the database to their local machine.
...
To expose a database to Common Fate, you will configure each database an role using the databases configuration block. Some examples have been provided below for configuring an mysql and postgres database.
...
data "aws_caller_identity" "current" {}
 ...
 module "proxy" {
  source    = "git::https://github.com/common-fate/terraform-aws-common-fate-proxy-ecs.git?ref=b0b65fd1eafb7672d5ceceb8b6fb55a46814c7fd"
