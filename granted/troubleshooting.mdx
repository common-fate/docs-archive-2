...
5. If you are still running into issues with assume and AWS SSO. We have a flag `--no-cache` which disables the persistence and lookup of cached tokens to force a fresh login and retrieving of a new access token. 
```bash
assume --no-cache

//also configurable via environment variable
export GRANTED_NO_CACHE 
```
