...
The adminsitrator role is referred to as a built-in role, and it can be accessed using a JIT assignment in the same way that users request access to other entitlements.
...
| name         | ID                        | access |
| ------------ | ------------------------- | ------ |
| Adminitrator | CF::Role::"administrator" | Manage policies, read deployment secrets, create integrations    |
...
permit (
    ...
    action == Access::Action::"Request",
    resource is CF::DeploymentEntitlement
) when {resource.role == CF::Role::"adminsitrator"};
